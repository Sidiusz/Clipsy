...
public static class Strings
{
    private static readonly Dictionary<string, Dictionary<string, string>> _all = new()
    {
        ...
        ["HelperVideoFormat"]   = New("Output format for saved recordings.", "Формат вывода для сохранённых записей."),
        ["LblJpgQuality"]       = New("JPEG quality",              "Качество JPEG"),
        ["LblScreenshotCursor"] = New("Include cursor",            "Включить курсор"),
        ["HelperScreenshotCursor"] = New("Draw mouse pointer on screenshots.", "Отображать курсор на скриншотах."),
        ...
        ["HelperVideoCursor"]   = New("Show mouse pointer in recorded video.", "Отображать курсор в записанном видео."),
        ["LblAfterSave"]        = New("After save",                "После сохранения"),
        ["HelperAfterSave"]     = New("Runs after every saved screenshot or recording.", "Срабатывает после каждого сохранённого скриншота или записи."),
        ...
        ["CodecH265Desc"]       = New("Smaller files, slower encode", "Меньше файлы, медленнее кодирование"),
        ["CodecVp9Desc"]        = New("Web-friendly, open codec",  "Дружественный к вебу, открытый кодек"),
        ["CodecAv1Desc"]        = New("Smallest, very slow encoder", "Наименьший размер, очень медленное кодирование"),
        ...
        ["TipRotateDragRegion"]   = New("Hold the move handle to drag a region while recording.", "Удерживайте ручку перемещения, чтобы двигать область во время записи."),
        ["TipRotateErase"]        = New("Right-click while drawing to erase.",              "ПКМ во время рисования — стирание."),
        ["TipRotateOcr"]          = New("Use the text tool to copy text out of any capture.", "Инструмент «текст» копирует текст с любого захвата."),
        ["TipRotateGif"]          = New("Pick GIF as the video format to export animations.", "Выберите формат GIF, чтобы экспортировать анимацию."),
        ["TipRotateHotkeys"]      = New("Rebind every shortcut under Hotkeys.",            "Переназначьте любые клавиши в разделе «Горячие клавиши»."),
        ["TipRotateLock"]         = New("Double-click a region to lock its size.",         "Двойной клик по области фиксирует её размер."),
