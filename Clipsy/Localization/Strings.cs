...
public static class Strings
{
    private static readonly Dictionary<string, Dictionary<string, string>> _all = new()
    {
        ...
        ["WarnCodecFallback"] = New("VP9 / AV1 not yet supported - recording in H.264.",
                                     "VP9 / AV1 пока не поддерживается, запись в H.264."),
        ["HelperCodecVp9Av1"] = New("VP9 / AV1 requires ffmpeg, which Clipsy does not bundle yet. Recording will fall back to H.264 until that is wired up.",
                                     "Для VP9 / AV1 нужен ffmpeg, который пока не входит в поставку Clipsy. Пока выбран этот кодек, запись идёт в H.264."),
